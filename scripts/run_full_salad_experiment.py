...
运行方式：
  python scripts/run_full_salad_experiment.py                    # 全量运行
  python scripts/run_full_salad_experiment.py --groups exp1,exp2 # 指定组
  python scripts/run_full_salad_experiment.py --max-samples 10   # 快速验证
  python scripts/run_full_salad_experiment.py --resume           # 断点续传
  python scripts/run_full_salad_experiment.py --models qwen2.5:1.5b  # 指定模型
"""
...
OUTPUT_DIR = ROOT_DIR / "output" / "full_experiment"

DATASETS = {
    "exp_a": DATA_DIR / "power_qa" / "defect_classification_150.jsonl",
    "exp_b": DATA_DIR / "power_ts" / "dq_scenarios_50.jsonl",
    "exp_c": DATA_DIR / "power_dispatch" / "dispatch_cmds_80.jsonl",
}
...
EXPERIMENT_GROUPS = {
    ...
    "exp1_traditional_poison": {
        ...
        "runs": [
            # (model, exp_name, prompt_dict_key, level, temperature, use_rag)
    ...
    "exp4_cross_model": {
        "description": "跨模型验证：SALAD增益与模型架构无关（所有可用本地模型）",
        "runs": [
            # 每个模型做 L0（基线）和 L2（SALAD最佳）对比
            *[
                run
    ...
    "exp10_scale_gradient": {
        "description": "规模梯度实验：0.8B→1.5B→3B→35B 的SALAD效果曲线",
        "runs": [
            # 规模梯度：各规模代表模型 × L0/L2
            ("qwen3.5:0.8b",   "exp_a", "EXP_A_SALAD", 0, 0.0, False),
            ("qwen3.5:0.8b",   "exp_a", "EXP_A_SALAD", 2, 0.0, False),
            ("qwen2.5:1.5b",   "exp_a", "EXP_A_SALAD", 0, 0.0, False),
            ("qwen2.5:1.5b",   "exp_a", "EXP_A_SALAD", 2, 0.0, False),
            ("qwen2.5:3b",     "exp_a", "EXP_A_SALAD", 0, 0.0, False),
            ("qwen2.5:3b",     "exp_a", "EXP_A_SALAD", 2, 0.0, False),
            ("qwen3.6:35b",    "exp_a", "EXP_A_SALAD", 0, 0.0, False),
            ("qwen3.6:35b",    "exp_a", "EXP_A_SALAD", 2, 0.0, False),
        ],
    },
    ...
    "exp13_salad_l3_rag": {
        ...
        "runs": [
            ("qwen2.5:1.5b", "exp_a", "EXP_A_SALAD", 2, 0.0, False),  # L2（对照）
            ("qwen2.5:1.5b", "exp_a", "EXP_A_SALAD", 3, 0.0, True),   # L3（+RAG）
            ("qwen2.5:1.5b", "exp_b", "EXP_B_SALAD", 2, 0.0, False),
    ...
    "exp15_sequential_decompose": {
        ...
        "runs": [
            # SALAD L2 单步（对照基准）
            ("qwen2.5:1.5b", "exp_a", "EXP_A_SALAD", 2, 0.0, False),
            # 两步分解（特殊处理，通过 run_type='seq' 标记）
            # 实际运行由 run_sequential_decompose() 函数处理
        ],
        "extra_seq": [
            ("qwen2.5:1.5b", "exp_a", 0.0),  # (model, exp, temperature)
        ],
    },
...
def setup_logging(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("salad_exp")
...
def build_prompt(
    ...
    """
    构建 prompt，返回 (system_prompt, user_prompt)
    """
...
def run_sequential_decompose(
    ...
    """两步推理实验：Step1 预测 defect_type，Step2 给定类型预测 severity"""
...
# ===========================================================
# 单次运行（model × exp × prompt × level × temperature）
# ===========================================================
...
def main():
    parser = argparse.ArgumentParser(description="SALAD 全量消融实验运行器")
    parser.add_argument("--groups", default="all",
                        help="要运行的实验组，逗号分隔（all 表示全部），例: exp1,exp2,exp13")
    parser.add_argument("--max-samples", type=int, default=-1,
                        help="每个数据集最多使用多少条样本（-1=全量）")
    parser.add_argument("--resume", action="store_true",
                        help="断点续传，跳过已完成的 run_id")
    parser.add_argument("--models", default="",
                        help="只使用这些模型（逗号分隔），不指定则使用实验组默认配置")
    parser.add_argument("--output-dir", default="",
                        help="输出根目录（默认: output/full_experiment）")
    parser.add_argument("--api-delay", type=float, default=0.8,
                        help="云端API调用间隔秒数（防止限流）")
    parser.add_argument("--skip-rag-init", action="store_true",
                        help="跳过RAG引擎初始化（加快启动，L3实验会降级为无RAG）")
    ...
    logger.info(f"SALAD 全量实验启动  output={out_root}  log={log_file}")
    logger.info(f"参数: groups={args.groups} max_samples={args.max_samples} "
                f"resume={args.resume} api_delay={args.api_delay}")
    ...
    logger.info(f"正在检查模型可用性: {all_models_needed}")
    available_models = set()
    for model in all_models_needed:
        if check_model_available(model, caller):
            available_models.add(model)
            logger.info(f"  ✓ {model}")
        else:
            logger.warning(f"  ✗ {model} (不可用，将跳过)")
    ...
    logger.info(f"\n{'='*60}")
    logger.info(f"全量实验完成！总计 {len(master_metrics)} 个运行")
